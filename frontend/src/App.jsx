...
function App() {
  ...
  return (
    <div className="h-screen bg-navy-950 text-slate-800 dark:text-slate-200 flex flex-col transition-colors duration-200 overflow-hidden">
      {/* ─────────── HEADER ─────────── */}
      <header className="w-full bg-slate-50/90 dark:bg-navy-900/80 backdrop-blur-xl sticky top-0 z-50 border-b border-slate-200 dark:border-navy-700 transition-colors">
                  ...
                  >
                    <div className="p-1">
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 px-3 py-2 text-[11px] font-bold text-danger bg-navy-900 border border-danger/50 hover:border-danger hover:bg-danger/10 rounded-lg transition-all"
                      >
      ...
      <main className="flex-1 flex overflow-hidden relative">
        {/* ─── GLOBAL SIDEBAR ─── */}
        <aside className={`shrink-0 absolute md:static z-20 left-0 top-0 h-full bg-slate-50 dark:bg-navy-900 border-r border-slate-200 dark:border-navy-700 transition-all duration-300 ease-in-out overflow-hidden flex flex-col ${sidebarOpen ? 'w-60 px-4 translate-x-0 opacity-100' : 'w-0 px-0 -translate-x-full md:translate-x-0 opacity-0 md:opacity-100'}`}>
          <div className="w-52 min-w-[13rem] py-6">
            ...
            <div className="space-y-1">
              ...
              <button onClick={() => { setView('admin'); setActiveAdminTab('requests'); if (window.innerWidth < 768) setSidebarOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-sm font-semibold ${view === 'admin' && activeAdminTab === 'requests' ? 'bg-accent/15 text-accent border border-accent/20' : 'text-slate-600 dark:text-slate-500 dark:text-slate-400 hover:bg-slate-200/50 dark:hover:bg-navy-700/50 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white border border-transparent'}`}>
                ...
                <span>{['ADMIN', 'MANAGER'].includes(user.user_role) ? (t.request_list_link || 'Request List') : (t.my_requests || 'My Requests')}</span>
              </button>
            </div>
          </div>
        </aside>
        ...
        {/* ─── CONTENT AREA ─── */}
        <div className="flex-1 min-w-0 px-4 md:px-6 py-6 overflow-y-auto bg-slate-100 dark:bg-transparent transition-all duration-300">
